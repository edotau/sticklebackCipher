...
BAM=$DIR/${PREFIX}.bam
bwa mem -t 8 $REF $1 $2 | samtools view -hb /dev/stdin | samtools sort -@ 8 -T $DIR/${PREFIX}.tmp -o $BAM
samtools index $BAM

bamToBwig=/data/lowelab/edotau/sticklebackCipher/atac/bamToBw.sh
sbatch $bamToBwig $BAM $REF
peakCaller=/data/lowelab/edotau/sticklebackCipher/atac/atacPeaks.sh
sbatch $peakCaller $BAM
